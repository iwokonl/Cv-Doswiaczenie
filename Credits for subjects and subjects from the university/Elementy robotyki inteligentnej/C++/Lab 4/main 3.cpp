...
int main()
{
	...
	if(NXT::OpenBT(&comm)) //initialize the NXT and continue if it succeeds
	{
		...
		*/
			...
			cout<<" current "<<NXT::Sensor::GetSonarValue(&comm, IN_2)<<endl;
			
			cout<<" TEMP "<<NXT::Sensor::GetSonarValue(&comm, IN_2) - 180<<endl;
			}
			if(decyzja=='p' || decyzja=='P'){
			int current = 2*NXT::Sensor::GetSonarValue(&comm, IN_4);
			int tolerancja = 3;

				
				if(current - temp >0 && !(current >= temp -10 && current <= temp+10)){
				NXT::Motor::SetForward(&comm,OUT_B,5);
				NXT::Motor::SetReverse(&comm,OUT_C,5);
				cout<<"lewa";
				}
				else if((current - temp)*-1 >0 && !(current >= temp -10 && current <=temp +10)){
